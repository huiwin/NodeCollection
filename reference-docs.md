...
| v2.6.2 | 2026-09-02 | patch | P14 subconverter 外部配置加载根因修复 (file:// 前缀 + [custom] section) |
...
| v2.8.0 | 2026-09-03 | minor | P16 综合订阅 (融合底座 + 主订阅有效节点补充)。T16.1 综合订阅：output/merged/ 升级为综合订阅 (订阅地址不变) — generate_merged_format 解析阶段补充主订阅有效节点 (新增 _load_main_sub_proxies 读取 output/clash/latest.yaml, 过滤违规词 + http/socks5 质量差协议), 统一走去重/测速/健康/剔除/排序/截断/重命名管线, 预期融合 73 + 主订阅补充 ~45 ≈ 118 节点。T16.2 index.json 标记综合订阅：merged.type=combined、merged.main_supplement、sources 增加主订阅 (含贡献统计)。T16.3 主页只显示综合订阅：README 订阅链接只展示 output/merged/ 各格式 (主订阅不再单独对外展示), status.html 更新为综合订阅 (含主订阅补充卡片, v2.8.0)。 |
| v2.8.1 | 2026-09-03 | patch | P16.1 修复 Clash 导入报错 "proxy [广告拦截] not found"：external_config.ini/merged_config.ini 的 ruleset 引用了 11 个代理组 (广告拦截/隐私防护/全球直连/国外加速/电报/苹果/微软/谷歌/国外媒体/国内媒体/漏网之鱼), 但 custom_proxy_group 只定义 15 个地区组导致规则引用组缺失 → Clash 校验失败；补全 ruleset 引用组 (15→26 组, 广告拦截→REJECT/全球直连→DIRECT/国外加速→自动选择等), 本地 subconverter 验证 rules 引用组全部匹配无缺失。 |
| v2.9.0 | 2026-09-03 | minor | P17 主订阅源健康治理 + 综合订阅补充修复：① 修复综合订阅主订阅补充丢失——线上主订阅含 !<str> 非标准 YAML tag (password: !<str> 1) 导致 yaml.safe_load 抛 ConstructorError → _load_main_sub_proxies 返回 [] → main_supplement=0；新增 _LenientYAMLLoader 忽略未知 tag 并全局应用于 _load_main_sub_proxies/classify_subscription/parse_upstream_text/_filter_and_rename_clash_file，容错解析订阅文件；② 主订阅源健康治理——classify_subscription 空壳过滤 (clash 需 proxies 非空、v2 需至少一条有效协议行)、新增 SUB_URL_BLACKLIST (ziyoufly/youzilite/xswl-fff/mtvpn/820010/knjc/xueshanlink/apit.yun7g 等失效/低质/违规源)、新增 _purge_stale_sub_urls 合并前验证旧列表防 sub/latest.yaml 累积失效源。端到端验证 main_supplement=41、sources 含主订阅恢复。 |
| v2.9.1 | 2026-09-04 | patch | P17.1 补充 sub_check 黑名单过滤：P17 部署验收发现 sub/latest.yaml 仍残留黑名单源 (ziyoufly/youzilite/820010/xswl-fff/knjc 等)——根因是 sub_check 本身未做黑名单判断, TG 频道新抓取的黑名单源仍被判定有效；修复 sub_check 开头匹配 SUB_URL_BLACKLIST 直接返回 None, check_all_urls 新结果与 _purge_stale_sub_urls 旧列表全部自动过滤。验证黑名单源直接拒绝、正常源正常验证。 |
| v2.10.0 | 2026-09-05 | minor | P18 订阅瘦身 + 代理组简约化 + 清理开心玩耍残留：① 订阅文件瘦身——根因是 rules 90,505 条 (ACL4SSR BanEasyList/BanEasyPrivacy 各数万条广告/追踪域名 + ChinaIp 7590 条 IP 段) 导致文件 4.96MB 加载卡顿；去掉 BanEasyList/BanEasyPrivacy (核心广告保留 BanAD)、ChinaIp 用 []GEOIP,CN 一行代替, 文件 4.96MB→132KB (38倍)、rules 90,505→2,221 条；② 代理组简约化 26→17 组——去掉隐私/电报/苹果/微软/谷歌/媒体/故障转移/负载均衡细分组, 保留核心 4 规则组 (广告拦截/全球直连/国外加速/漏网之鱼) + 13 地区组, 细分规则合并 (电报/谷歌/国外媒体→加速, 苹果/微软/国内媒体→直连), 规则引用组全部匹配无缺失；③ 开心玩耍 play_list 过滤 SUB_URL_BLACKLIST 源清理展示残留。 |
| v2.11.0 | 2026-09-07 | minor | P19 补充优质订阅源：① 融合订阅 merge.yaml 新增 3 个上游——getNode (a2470982985/getNode, 每小时更新聚合源, Clash YAML 格式直接兼容)、abshare (每日公益流量)、mksshare/Mikasa (每日公益流量), 均走 max_nodes 截断 + 测速健康剔除 + 违规词重命名 + 黑名单过滤保护, UPSTREAM_REPO_MAP 同步 README 来源表致谢；② 主订阅新增固定订阅源 config.yaml fixed_subscriptions (getNode v2ray 格式/free18/aiboboxx), main.py 新增 load_fixed_subscriptions() 并入主订阅 sub_check 校验流程, 与 TG 频道/机场同等走有效性校验, 失效自动剔除。 |
...
| 2026-09-07 | 实施 v2.11.2 P19.2 移除空文件源：P19.1 验收 getNode 仍未进入上游统计 (上游 6 个缺 getNode, 固定源未通过 sub_check), 深入排查——jsdelivr API 确认 getNode 仓库文件树 clash.yaml 与 v2ray.txt 均为 0 字节 (镜像返回 200 但 len=0, 生成脚本 run.sh 未输出节点), 该源当前不可用; 实施移除 getNode 上游 + 移除 getNode 固定源, merge.yaml 回到 5 稳定上游, 保留逐行 base64 解码兼容能力 (验证 3/3 通过但当前无源使用); P19 全链条结论: GitHub 免费订阅源 (getNode 空文件/abshare-mksshare 403 Cloudflare/free18 vless 协议被 P11.6 过滤且主订阅协议白名单不含/free18 自建节点单条/free18 与 aiboboxx 停更) 均无法纳入, 主订阅可靠来源仍是 TG 频道 35 个 + 机场探测 40 个, 综合订阅 147 节点/100% 可用率保持稳定。 |
| 2026-09-07 | 实施 v2.11.1 P19.1 订阅源修正：P19 部署验收发现 getNode/abshare/mksshare 未进入上游统计、固定源未通过校验——逐项排查根因——① getNode clash.yaml 是纯规则文件 (无 proxies 段, parse 0 节点), 正确节点在 v2ray.txt (每行独立 base64 编码 vmess URI, 整体解码不可靠), 修复 merge.yaml 改 v2ray.txt 并在 parse_upstream_text/classify_subscription 增加逐行 base64 解码回退 (本地 3/3 行验证通过)；② abshare/mksshare 返回 403 (Cloudflare 防护, 本地 web_fetch/镜像均不可达) 移除；③ free18 节点为 vless (P11.6 过滤 + 主订阅协议白名单不含 vless) 不加入；④ aiboboxx README 停更 2023-09 移除；config.yaml 固定源精简为 getNode v2ray.txt。 |
| 2026-09-07 | 实施 v2.11.0 P19 补充优质订阅源：用户要求补充优质主订阅源 (T17.2 暂缓部分), 调研验证——getNode (a2470982985/getNode) 每小时更新 1.3k stars 最活跃且 Clash YAML 格式兼容确认可访问, abshare/mksshare 每日公益流量仓库活跃 (本地网络不可达, 线上 Actions 自动验证); free18/aiboboxx 仓库活跃但需线上验证；实施——merge.yaml 新增 getNode (clash.yaml)/abshare/mksshare 3 个融合上游 (max_nodes 80, 走测速过滤+违规词重命名+健康剔除保护), main.py UPSTREAM_REPO_MAP 同步; config.yaml 新增 fixed_subscriptions 固定订阅源 (getNode v2ray.txt/free18 c/v2ray/aiboboxx v2), main.py 新增 load_fixed_subscriptions() 并入主订阅 sub_check 校验流程, 失效自动剔除不污染。 |
| 2026-09-05 | 实施 v2.10.0 P18 订阅瘦身 + 代理组简约化 + 清理开心玩耍残留：用户反馈订阅文件过大 (4.96MB) 导致 Clash 加载不出来, 分析确认 rules 90,505 条 (BanEasyList 数万条广告域名 + BanEasyPrivacy 数万条追踪域名 + ChinaIp 7590 条 IP-CIDR) 占文件 3.4MB 是主因；修复——两 config 去掉 BanEasyList/BanEasyPrivacy (核心广告保留 BanAD ~3000 条)、ChinaIp 用 []GEOIP,CN 一行代替 (Clash 内置 GeoIP 判断国内直连), 文件 4.96MB→132KB (38倍)、rules 90,505→2,221 条；同时代理组简约化 26→17 组——去掉隐私/电报/苹果/微软/谷歌/国外媒体/国内媒体/故障转移/负载均衡细分组, 保留核心 4 规则组 + 13 地区组 (地区选择/自动选择/11 地区), 细分规则合并到核心组 (电报/谷歌/国外媒体→国外加速, 苹果/微软/国内媒体→全球直连) 不牺牲分流质量；本地 subconverter 验证 17 组/132KB/规则引用组全部匹配无缺失；开心玩耍 play_list 过滤黑名单源。 |
| 2026-09-04 | 实施 v2.9.1 P17.1 补充 sub_check 黑名单过滤：P17 部署验收发现 sub/latest.yaml 机场订阅仍含 youzilite/ziyoufly/xswl-fff/knjc、clash订阅仍含 820010 (均为 P17 SUB_URL_BLACKLIST 内源), 根因是 sub_check 本身未做黑名单判断, TG 频道新抓取的黑名单源 (sub_check 只做空壳/类型判断) 仍被判定有效并合并进 sub/latest.yaml；修复——sub_check 开头匹配 SUB_URL_BLACKLIST 直接返回 None, 使 check_all_urls 新结果与 _purge_stale_sub_urls 旧列表统一自动过滤；验证黑名单源直接拒绝 (不发请求)、正常源正常验证。 |
| 2026-09-03 | 实施 v2.9.0 P17 主订阅源健康治理 + 综合订阅补充修复：发现并修复线上综合订阅主订阅补充丢失 (main_supplement=0) 的 bug——主订阅 output/clash/latest.yaml 含 !<str> 非标准 YAML tag (password: !<str> 1), yaml.safe_load 抛 ConstructorError 导致 _load_main_sub_proxies 返回 []；新增 _LenientYAMLLoader 忽略未知 tag 并应用于 _load_main_sub_proxies/classify_subscription/parse_upstream_text/_filter_and_rename_clash_file (全局容错)；同时做主订阅源健康治理——classify_subscription 空壳过滤 (clash proxies 非空/v2 至少一条有效协议行)、SUB_URL_BLACKLIST (ziyoufly/youzilite/xswl-fff/mtvpn/820010/knjc/xueshanlink/apit.yun7g 等)、_purge_stale_sub_urls 合并前验证旧列表防失效源累积。单元测试 6 项全过, 端到端 main_supplement=41/sources 含主订阅, 兜底重命名不再失败。 |
| 2026-09-03 | 实施 v2.8.1 P16.1 修复 Clash 导入报错 "proxy [广告拦截] not found"：P16 综合订阅部署后用户导入 Clash Verge 报错 (rules[0] [DOMAIN-KEYWORD,admarvel,广告拦截] error: proxy [广告拦截] not found), 根因是 external_config.ini/merged_config.ini 的 ruleset 引用了 11 个代理组但 custom_proxy_group 只定义 15 个地区组, 规则引用组缺失导致 Clash 校验失败；修复——两个 config 补全 ruleset 引用组 (15→26 组, 广告拦截/隐私防护→REJECT、全球直连/微软→DIRECT、国外加速/谷歌/国外媒体/漏网之鱼→自动选择、国内媒体→DIRECT/自动、苹果/电报→自动), 本地 subconverter 验证 proxy-groups 26 个、rules 引用组全部匹配无缺失；版本号 v2.8.0→v2.8.1 |
| 2026-09-03 | 实施 v2.8.0 P16 综合订阅 (融合底座 + 主订阅有效节点补充)：T16.1 output/merged/ 升级为综合订阅 (订阅地址不变)——generate_merged_format 解析阶段新增主订阅补充 (新增 _load_main_sub_proxies 读取主订阅 output/clash/latest.yaml, 过滤违规词 + http/socks5 质量差协议), 补充节点统一走去重/测速/健康/剔除/排序/截断/重命名管线, 预期融合 73 + 主订阅补充 ~45 ≈ 118 节点; T16.2 index.json 标记 merged.type=combined + main_supplement + sources 增加主订阅来源; T16.3 主页只显示综合订阅——README 订阅链接只展示 output/merged/ 各格式 (主订阅不再单独对外展示, 仍作为内部数据源), status.html 更新为综合订阅含主订阅补充卡片; 版本号 v2.7.0→v2.8.0 |
...
| 2026-09-03 | 实施 v2.7.0 P15 主订阅转换持续失败根因修复：失效/空壳订阅拖垮整体——filter_valid_sub_urls 合并测试优先+失败逐个剔除; 主订阅 emoji 节点名被 remove_old_emoji 拆成 int, 改回 _rename_with_region; external_config.ini 删除冲突 rename 规则+扩充 emoji/加拿大组; 效果: 主订阅 87 节点/15 地区组/0 违规词 |
| 2026-09-03 | 实施 v2.8.0 P16 综合订阅架构 (融合为底座 + 主订阅有效节点补充)：README/status 主页只展示综合订阅 5 格式; generate_merged_format 增加主订阅补充逻辑 (main_supplement); 对外唯一推荐综合订阅 |
| 2026-09-04 | 实施 v2.9.0 P17 主订阅源健康治理 + 综合订阅补充修复：_LenientYAMLLoader 忽略未知 YAML tag (!<str>) 修复补充读取失败; 主订阅源健康状态跟踪; 版本号 v2.8.0→v2.9.0 |
| 2026-09-04 | 实施 v2.9.1 P17.1 补充逻辑修正 (综合订阅 main_supplement 恢复) |
| 2026-09-05 | 实施 v2.10.0 P18 订阅瘦身 + 代理组简约化：根因 ACL4SSR BanEasyList/BanEasyPrivacy (各数万条) + ChinaIp (7590 条 IP-CIDR) 占 3.4MB; 修复: 两 config 去 BanEasyList/BanEasyPrivacy (保留 BanAD)、ChinaIp 用 []GEOIP,CN 一行代替、代理组 26→17 (4 核心规则组 + 13 地区组)、开心玩耍 play_list 过滤黑名单; 效果: 4.96MB→133KB、rules 90505→2221、规则引用组无缺失 |
| 2026-09-05 | 实施 v2.11.0 P19 补充优质主订阅源：merge.yaml +3 上游 (getNode/abshare/mksshare) + config.yaml +3 固定源 + load_fixed_subscriptions(); 部署验收发现新源全未进统计 |
...
| 2026-09-15 | 实施 v2.14.1 P24 扩充上游源：源质量持续退化（9/15 可用率 37-45%、输出仅 40-54 节点）；逐个验证候选源内容后整合 free18/v2ray（c.yaml 385 节点，trojan 300+vmess 5，可输出 ~305）与 free-nodes/v2rayfree（base64 1637 行，ss 556+vmess 517+trojan 15，可输出 ~1088），vless 自动过滤；abshare/OpenRunner 无订阅文件、mksshare/dongyubin 404、vxiaov 内容少均未采用；版本升至 v2.14.1 |
| 2026-09-14 | 实施 v2.14.0 P23.1 修复：告警冷却状态持久化到 index.json 顶层 alert_state（初始版用内存 dict，Actions 每轮全新进程导致冷却从未生效、持续刷屏；改为跨周期持久化，发送成功才记录，warning 6h / error 1h / info 不冷却）；版本号保持 v2.14.0 |
| 2026-09-13 | 实施 v2.14.0 P23 告警冷却去重：同类告警限频（可用率持续 <50% 时每轮 cron 重复触发刷屏，Server酱每日 5 条配额被耗尽）；按 (title, level) 去重，warning 6h / error 1h / info 不冷却；版本号保持 v2.14.0 |
| 2026-09-10 | 实施 v2.14.0 P22.3 根因修复（14 分钟问题）：日志确诊校验阶段 1431 URL 前 1423 个仅 34s、最后 8 个卡 12 分钟（单 URL DNS 解析/连接挂起 11.5 分钟，getaddrinfo 不受 requests timeout 控制）；check_all_urls 与 _purge_stale_sub_urls 加阶段硬超时护栏 CHECK_ALL_TIMEOUT=150s（as_completed/executor.map timeout + shutdown wait=False 放弃残留线程），慢 URL 影响从 12 分钟锁死到 150s；版本号保持 v2.14.0 |
| 2026-09-10 | 实施 v2.14.0 P22.2 诊断增强：main() 新增 9 个阶段计时日志 [timer]（定位 #206 耗时 14.3 分钟异常；嫌疑：上游 60s×3 重试链 / subconverter 11 次调用×60s 超时；正常 2-3 分钟）；版本号保持 v2.14.0 |
